...
void SendByteSPI(uint8_t byte) {
    for (int i = 0; i < 8; i++) {
        if ((byte << i) & 0x80) {
            ...
            GPIO_WriteBit(SID_PORT, SID_PIN, SET);
        }

        else
            GPIO_WriteBit(SID_PORT, SID_PIN, RESET);  // SID=0

        GPIO_WriteBit(SCLK_PORT, SCLK_PIN, RESET);  // SCLK =0  OR SCK

        GPIO_WriteBit(SCLK_PORT, SCLK_PIN, SET);  // SCLK=1
...
void ST7920_SendCmd(uint8_t cmd) {
    ...
    GPIO_WriteBit(CS_PORT, CS_PIN, SET);  // PUll the CS high
    ...
    GPIO_WriteBit(CS_PORT, CS_PIN, RESET);  // PUll the CS LOW
...
void ST7920_Clear() {
    if (Graphic_Check == 1)  // if the graphic mode is set
        ...
        Delay_Ms(2); // delay >1.6 ms
    }
}

void ST7920_Init(void) {
    GPIO_WriteBit(RST_PORT, RST_PIN, RESET);  // RESET=0
    Delay_Ms(10);   // wait for 10ms
    GPIO_WriteBit(RST_PORT, RST_PIN, SET);  // RESET=1
